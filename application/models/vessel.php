<?php
	...
	class Vessel extends CI_Model
	{
		...
		public function delete($Id)
		{
			...
			{
				$file_path = array();
				$sql = "SELECT attachment.FilePath as FilePath FROM vessel LEFT JOIN vesselvoyage ON vessel.Id = vesselvoyage.VesselId LEFT JOIN attachment ON vessel.Id = attachment.ReferenceId WHERE vessel.Id ='".$Id."'";
				$query = $this->db->query($sql);
				if($query)
				{
					file_put_contents(BASEPATH.'logs.txt', date("Y-m-d H:i:s")."\t".$sql."\n", FILE_APPEND);
